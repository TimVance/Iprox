...
class CKDAImportExcel {
	...
	public function SaveRecord($arItem)
	{
		...
		$allowCreate = (bool)($dbRes->SelectedRowsCount()==0 && $this->params['ONLY_DELETE_MODE']!='Y');
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array())
	{
		...
		if($dbRes->SelectedRowsCount()==0 && $ID && $this->params['SEARCH_OFFERS_WO_PRODUCTS']!='Y')
		{
			if($this->params['ONLY_UPDATE_MODE_ELEMENT']!='Y')
			{
				...
				if($this->CheckRequiredProps($arFieldsProps, $OFFERS_IBLOCK_ID) && ($OFFER_ID = $this->AddElement($arFieldsElement)))
				{
		...
		elseif(preg_match('/http(s)?:\/\//', $file))
		{
			...
			if(class_exists('\Bitrix\Main\Web\HttpClient'))
			{
				...
				try{
					...
					$file = preg_replace_callback('/[^:\/?=&#@]+/', create_function('$m', 'return rawurlencode($m[0]);'), str_replace('\\', '/', $file));
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $needUpdate = false)
	{
		...
		foreach($arProps as $k=>$prop)
		{
			...
			if($propsDef[$k]['PROPERTY_TYPE']=='S' && $propsDef[$k]['USER_TYPE']=='video')
			{
