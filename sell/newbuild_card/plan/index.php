<?
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
$APPLICATION->SetTitle("АК &ldquo;Горизонт&rdquo; — планировки");
?>

			<div class="adress-agency">Сочи, Адлерский район, Адлер-центр, ул. Просвещения, 24</div>
      
			<!--тут твоя верстка-->
			  <div class="sales-map">
				<div class="sales-polygon">
				  <div class="cloth-wrap">
					<div class="tree"></div>
					<table class="cloth">
					  <tr>
						<th>14</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>13</th>
						<td></td>
						<td></td>
						<td class="filled">
						  <span>1</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td class="filled">
						  <span>1</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>12</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>11</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>10</th>
						<td></td>
						<td></td>
						<td class="filled">
						  <span>1</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>9</th>
						<td></td>
						<td class="filled">
						  <span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>8</th>
						<td></td>
						<td class="filled">
						  <span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>7</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td class="filled">
						  <span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td class="filled">
						  <span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
					  </tr>
					  <tr>
						<th>6</th>
						<td class="filled">
						<span>1</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td class="filled">
						<span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>5</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					  <tr>
						<th>4</th>
						<td></td>
						<td></td>
						<td class="filled">
						<span>1</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
						<td class="filled">
						<span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
					  </tr>
					  <tr>
						<th>3</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td class="filled">
						<span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td></td>
					  </tr>
					  <tr>
						<th>2</th>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						<td class="filled">
						<span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
						<td class="filled">
						<span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div>
						</td>
					  </tr>
					  <tr>
						<th>1</th>
						<td></td>
						<td class="filled">
						  <span>2</span>
						  <div class="info">
							<span class="item_title">2-х комнатная квартира 60 м<sup>2</sup></span>
							<span class="item_price">3 600 000 руб.</span>
							<span class="item_per_meter">60 000 руб./м<sup>2</sup></span>
							<div class="item_preview"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/sales_poly_item1.jpg" alt=""></div>
						  </div></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
					  </tr>
					</table>
				  </div>
				  <ul class="exps">
					<li>
					  <div class="clr grey"></div>
					  <span class="txt">Продано</span>
					</li>
				  </ul>
				</div>
				<div class="floorplans">
				  <div class="floorplans-in">
					<div class="tabs-tb">
								
					  <div class="nav-regist nav-tb">
						<span class="nav-title">Планы этажей</span>
						<ul>
						  <li class="active"><a href="#">1 этаж</a></li>
						  <li><a href="#">Типовой</a></li>
						</ul>
					  </div><!--nav-regist-->
					  
					  <div class="cont-tb">
						<div class="tab-tb">
						  <img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/floorplan1.png" alt="">
						</div>
						<div class="tab-tb">
						  <img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/floorplan1.png" alt="">
						</div>
					  </div><!--cont-tb-->
					</div><!--tabs-tb-->
				  </div>
				</div>
			  </div>
			  
			<div class="all-propos sm">
				<div class="t-emploe-wrap">
				<div class="t-emploe">Все предложения застройщика/инвестора  <span>164</span></div>
				<div class="sel-on">
					<select>
						<option>Руб</option>
						<option>Руб</option>
						<option>Руб</option>
						<option>Руб</option>
						<option>Руб</option>
					</select>
				</div><!--sel-on-->
			</div>
				
			<div class="table-history table-history2 bh">
				<table>
					<tr>
						<th><a class="active" href="#">Предложение</a>	<i>&uarr;</i></th>
						<th><a href="#">Площадь</a></th>
						<th><a href="#">Этаж</a></th>
						<th><a href="#">Цена</a></th>
						<th><a href="#">Цена м<sup>2</sup></a></th>
						<th><a href="#">Номер квартиры</a></th>
						<th><a href="#">Статус</a></th>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>26.1</strong></td>
						<td>1</td>
						<td><strong>1 488 000</strong></td>
						<td>57 011</td>
						<td>23</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>45.8</strong></td>
						<td>5</td>
						<td><strong>3 800 000</strong></td>
						<td>82 969</td>
						<td>30</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>3-комнатная</td>
						<td><strong>35.2</strong></td>
						<td>3</td>
						<td><strong>1 936 000</strong></td>
						<td>54 999</td>
						<td>28</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>2-комнатная</td>
						<td><strong>30</strong></td>
						<td>5</td>
						<td><strong>1 290 000</strong></td>
						<td>43 000</td>
						<td>29</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>110</strong></td>
						<td>4</td>
						<td><strong>9 500 000</strong></td>
						<td>86 363</td>
						<td>2</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>60.4</strong></td>
						<td>2</td>
						<td><strong>2 718 000</strong></td>
						<td>45 000</td>
						<td>16</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>3-комнатная</td>
						<td><strong>36</strong></td>
						<td>2</td>
						<td><strong>3 276 000</strong></td>
						<td>91 000</td>
						<td>10</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>2-комнатная</td>
						<td><strong>49.4</strong></td>
						<td>3</td>
						<td><strong>5 065 000</strong></td>
						<td>102 530</td>
						<td>4</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>36.1</strong></td>
						<td>6</td>
						<td><strong>2 200 000</strong></td>
						<td>60 941</td>
						<td>18</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>55.2</strong></td>
						<td>4</td>
						<td><strong>9 500 000</strong></td>
						<td>172 101</td>
						<td>40</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>3-комнатная</td>
						<td><strong>66</strong></td>
						<td>7</td>
						<td><strong>5 280 000</strong></td>
						<td>80 000</td>
						<td>51</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>2-комнатная</td>
						<td><strong>66.2</strong></td>
						<td>3</td>
						<td><strong>4 965 000</strong></td>
						<td>75 000</td>
						<td>26</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>36.1</strong></td>
						<td>6</td>
						<td><strong>2 200 000</strong></td>
						<td>60 941</td>
						<td>18</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>1-комнатная</td>
						<td><strong>55.2</strong></td>
						<td>4</td>
						<td><strong>9 500 000</strong></td>
						<td>172 101</td>
						<td>40</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>3-комнатная</td>
						<td><strong>66</strong></td>
						<td>7</td>
						<td><strong>5 280 000</strong></td>
						<td>80 000</td>
						<td>51</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>2-комнатная</td>
						<td><strong>66.2</strong></td>
						<td>3</td>
						<td><strong>4 965 000</strong></td>
						<td>75 000</td>
						<td>26</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>3-комнатная</td>
						<td><strong>66</strong></td>
						<td>7</td>
						<td><strong>5 280 000</strong></td>
						<td>80 000</td>
						<td>51</td>
						<td>в продаже</td>
					</tr>
					<tr>
						<td>2-комнатная</td>
						<td><strong>66.2</strong></td>
						<td>3</td>
						<td><strong>4 965 000</strong></td>
						<td>75 000</td>
						<td>26</td>
						<td>в продаже</td>
					</tr>
				</table>
			</div><!--table-history-->
		</div>
      
      <div class="block-developer">
		<div class="b-investor">
			<div class="t-investor">Застройщик/Инвестор</div>
			<div class="link-investor">
				<a href="#">АК "Санторини Клуб"</a>
				<span>Сочи, ул. Пирогова</span>
			</div>
			
			<div class="view-ph">
				<div class="view-phone">
					<a href="#">Показать телефон</a>
					<div class="phone-v"><span>89856523344</span></div>
				</div>
				
				<div class="send-mess"><a class="inline" href="#pop8">Оставить сообщение</a></div>
			</div>
		</div><!--b-investor-->
		
		<div class="contacts-face">
			<div class="t-face">Контактные лица</div>
			<div class="info-cont-rielt">
				<div class="in-emp">
					<div class="img-emp"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/img-emploe1.jpg" alt="" /></div>
					<div class="desc-emploe">
						<div class="name-emploe">
							Олег Александрович Коротких
							<span>Специалист по продаже недвижимости</span>
						</div>
						<div class="view-rielt-phone">
							<a href="#">Показать телефон</a>
							<span>89856523344</span>
						</div><!--view-rielt-phone-->
					</div><!--desc-emploe-->
				</div>
			</div>
			<div class="info-cont-rielt">
				<div class="in-emp">
					<div class="img-emp"><img src="<?=SITE_TEMPLATE_PATH?>/images_tmp/img-emploe1.jpg" alt="" /></div>
					<div class="desc-emploe">
						<div class="name-emploe">
							Олег Александрович Коротких
							<span>Специалист по продаже недвижимости</span>
						</div>
						<div class="view-rielt-phone">
							<a href="#">Показать телефон</a>
							<span>89856523344</span>
						</div><!--view-rielt-phone-->
					</div><!--desc-emploe-->
				</div>
			</div>
		</div><!--contacts-face-->
      </div>

<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/footer.php");?>