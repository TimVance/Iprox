...
$APPLICATION->SetTitle("Отключение пакетов пользователям");
...
if ($USER->IsAdmin())
{
			...
			foreach ($inputedFields[$Fields["USERS"]] as $UserID)
			{
				if ($UserID)
				{
					foreach ($inputedFields[$Fields["PACKETS"]] as $PacketID)
					{
						if ($PacketID)
						{
							\MorealtySale\Packets::removePacketFromUser($PacketID, $UserID);
	...
	$aTabs = array(
			array("DIV" => "settings", "TAB" => "Отключение пакетов пользователям", "TITLE"=>"Отключение пакетов пользователям"),
	);
  ...
  <tr>
    <td width="40%" style="vertical-align: top;">
        <span id="hint_userpackets"></span> Пользователи
			...
			BX.hint_replace(BX('hint_userpackets'), '<?=CUtil::JSEscape("У данных пользователей будут отключенны указанные пакеты")?>');
			</script>
    </td>
...
if ($isAdded)
	CAdminMessage::ShowNote("Пакеты отключенны");
