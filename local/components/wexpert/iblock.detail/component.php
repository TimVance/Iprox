 ...
 * Измения:
 * 
 * Своства привязки к элементам выполняют доп запрос и получают эти элементы
 * 
 * @version 2.4.1
...
if ($this->StartResultCache(false)) {
	...
	//changed
	$isEmpty = $arParams['SHOW_EMPTY_PROPERTY'] == 'Y' ? array() : array("EMPTY" => 'N');
		...
		if ($arParams['GET_PROPERTY'] == "Y") {
			...
			while ($ar_props = $db_props->GetNext()) {
				if ($ar_props['PROPERTY_TYPE'] == "E") {
					...
					if ($objEle = $res->GetNextElement())
					{
						$ar_res = $objEle->GetFields();
						$ar_res["PROPERTIES"] = $objEle->GetProperties();
