...
if ($this->StartResultCache(false)) {
	...
	// QUERY 1 MAIN
	$rsItems = CIBlockElement::GetList($arOrder, $arFilter, $arGroup, $arNavParams, $arSelect);
	//Финт чтобы избегать дублей и отдавать в том же формате
	if ($arParams["USE_GETNEXTELEMENT"] === true)
	{
		while ($objItem = $rsItems->GetNextElement()) {
			...
			foreach ($props as $propCode => $arProp)
			{
				if ($arProp["MULTIPLE"] == "Y" && $arProp["VALUE"])
				{
					$propToadd = $arProp;
					$propToadd["VALUE"] = false;
					foreach ($arProp["VALUE"] as $ValueKey => $arValue)
					{
						$newProp = $propToadd;
						$newProp["VALUE"] = $arValue;
						$arItem["PROPERTIES"][$propCode][$ValueKey] = $newProp;
					}
				}
				else 
				{
					$arItem["PROPERTIES"][$propCode] = $arProp;
				}
			}
