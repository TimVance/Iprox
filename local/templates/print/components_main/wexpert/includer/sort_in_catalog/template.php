	...
	<div class="sort catalog-sort">
		...
		<ul>
			...
			<? if($APPLICATION->GetCurPage() == '/builders/'): ?>
				<li value="PROPERTY_COUNT_OF_EMPLOYEES"><span class="<? echo (($_GET["SORT_BY"] == "PROPERTY_COUNT_OF_EMPLOYEES") ? 'active' : 'waited');?>">Количество сотрудников</span></li>
			<? endif; ?>
		</ul>
	</div><!--sort-->
	<? if ($arParams["MAP_LINK"])
	{
		?>
			<div class="link-map"><a href="<?=$arParams["~MAP_LINK"]?>">Карта</a></div>
		<?
	}?>
	<? if ($arParams["BACK_LINK"])
	{
		?>
			<div class="link-map"><a href="<?=$arParams["~BACK_LINK"]?>">Список</a></div>
		<?
	}?>
