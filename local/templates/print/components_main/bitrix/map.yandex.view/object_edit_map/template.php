...
$this->addExternalJS($templateFolder."/custom_function.js");
$this->addExternalJS($templateFolder."/ymapsstyle.js");
...
<?
	...
	if ($arParams["MAP_VERSION"])
	{
		$arTransParams["YANDEX_VERSION"] = $arParams["MAP_VERSION"];
	}
?>
...
function BX_SetPlacemarks_<?echo $arParams['MAP_ID']?>(map)
{
	...
	<?if ($arParams['EventsFunction']):
		?>
			if (window.<?echo $arParams['EventsFunction']?>)
			{
				window.<?echo $arParams['EventsFunction']?>(map, arObjects,<?=CUtil::PhpToJSObject($arParams["START_POINT"])?>);
