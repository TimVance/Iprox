...
<div id="ask_question-form" class="ask_question-form_at_builders_page">
        ...
        <form name="SIMPLE_FORM_<?=$arParams['WEB_FORM_ID']?>">
            ...
            foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
                if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden') {
                    ...
                    if ($type == 'textarea') {?>
                        <div class="field-ask_element textarea-pop"><textarea name="<?=$name?>" placeholder="<?=$placeholder?>"><?=$value?></textarea></div>
                    <?} else {?>
                        <? if($name == 'form_text_8'): ?>
                            <div class="field-ask_element field-pop field-pop4 hidden"><input type="text" name="<?=$name?>" placeholder="<?=$placeholder?>" value="<?=$arParams['elementID']?>" /></div>
                        <? else: ?>
                            <div class="field-ask_element field-pop field-pop4"><input type="text" name="<?=$name?>" placeholder="<?=$placeholder?>" value="<?=$value?>" /></div>
                        <? endif; ?>                    <?}?>
                    <?
                }
            } //endwhile
