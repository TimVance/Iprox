...
foreach ($arResult["IBLOCKS"] as $iblockKey => &$iblock)
{
	...
	{
		if (!in_array($iblock["ID"], $arParams["IBLOCK_IDS"]))
		{
			unset($arResult["IBLOCKS"][$iblockKey]);
			continue;
		}
	}
	if ($arParams["IBLOCK_URL_TEMPLATE"])
	{
		$iblock["LIST_PAGE_URL"] = \Morealty\Catalog::proccessIblockUrl($arParams["IBLOCK_URL_TEMPLATE"], $iblock);
		if ($arParams["IBLOCK_ID"] == $iblock["ID"])
		{
			$iblock["CURRENT"] = "Y";
		}
	}
	if ($arParams["ISMAP"] === "Y")
	{
		if ($APPLICATION->GetCurPage(false) == $iblock["MAP_PAGE_URL"])
		{
			$iblock["CURRENT"] = "Y";
		}
	}
	...
	{
		if (CSite::InDir($iblock["LIST_PAGE_URL"]))
		{
			$iblock["CURRENT"] = "Y";
		}
	}
...
//my_print_r(abs(crc32("4569")));
//my_print_r($arResult["ITEMS"]);
$district = $microdistrict =  array();
$districtKey = false;
foreach ($arResult["ITEMS"] as $key => $arItem)
{
	...
	if ($arItem["CODE"] == "microdistrict")
	{
		$microdistrict = $arItem;
	}
	else if ($arItem['CODE'] == "district")
	{
		$district = $arItem;
		$districtKey = $key;
	}
...
$this->__component->SetResultCacheKeys(array("PROCCESS_PROPS", "IBLOCKS"));
