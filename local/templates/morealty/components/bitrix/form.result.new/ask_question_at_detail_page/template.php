...
<div id="ask_question-form">
    <?if ($arResult["isFormNote"] != "Y") {?>
        ...
        if ($arResult["isFormTitle"]) {?>
            <div class="t-form"><?=$arResult["FORM_TITLE"]?></div>
            <? } ?>
        ...
        <form name="SIMPLE_FORM_<?=$arParams['WEB_FORM_ID']?>">
            ...
            foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
                if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden') {
                    ...
                    if ($type == 'textarea') {?>
                        <div class="textarea-ask"><textarea name="<?=$name?>" placeholder="<?=$placeholder?>"><?=$value?></textarea></div>
                    <?} else {?>
                        <? if($name == 'form_text_8'): ?>
                            <div class="field-ask hidden"><input type="text" name="<?=$name?>" placeholder="<?=$placeholder?>" value="<?=$arParams['elementID']?>" /></div>
                        <? else: ?>
                            <div class="field-ask"><input type="text" name="<?=$name?>" placeholder="<?=$placeholder?>" value="<?=$value?>" /></div>
                        <? endif; ?>                    <?}?>
                    <?
                }
            } //endwhile
